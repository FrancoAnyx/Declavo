...
interface NotifMessage {
  ...
  body: string
  created_at: string
  is_mine: boolean // si yo fui quien envió
}
...
export default function NotificationBell() {
  ...
  const fetchUnread = useCallback(async () => {
    ...
    // Mensajes sobre productos de NUESTRA empresa, enviados por OTROS, no leídos por nosotros
    const { data } = await supabase
      ...
      .eq('product_org_id', orgId)           // sobre nuestros productos
      .neq('sender_org_id', orgId)           // que no enviamos nosotros
    ...
    const msgs: NotifMessage[] = (data as {
      ...
      sku: m.sku,
      sender_org_name: m.sender_org_name,
    ...
    // Primera carga: registrar IDs existentes sin mostrar toast
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'product_messages' }, () => {
        fetchUnread()
      })
      .subscribe()
          ...
          <div style={{
            ...
            <div style={{ padding: '14px 16px 10px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <p style={{ margin: 0, fontWeight: 700, fontSize: 13, color: 'var(--text-primary)' }}>
                Consultas recibidas {count > 0 && <span style={{ color: 'var(--accent)' }}>({count})</span>}
              </p>
              {count > 0 && (
            ...
            {unread.length === 0 ? (
              <div style={{ padding: '28px 16px', textAlign: 'center', color: 'var(--text-muted)' }}>
                ...
                <p style={{ margin: 0, fontSize: 13 }}>Sin consultas nuevas</p>
              </div>
            ) : (
              unread.map(m => (
                <div key={m.id} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ margin: '1px 0 3px', fontSize: 11, color: 'var(--accent)', fontWeight: 600 }}>
                      {m.sku}
                    </p>
            ...
            {/* Link al centro de chats */}
            <div style={{ padding: '10px 16px', borderTop: unread.length > 0 ? '1px solid var(--border)' : 'none' }}>
