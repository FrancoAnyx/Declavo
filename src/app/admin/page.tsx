...
function InvitacionesSection() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const { data } = await supabase.from('invitations').select('*, organizations(name)').order('created_at', { ascending: false })
    setInvitations((data ?? []).map((i: Invitation & { organizations: { name: string } | null }) => ({ ...i, org_name: i.organizations?.name ?? '—' })))
    setLoading(false)
  }, [])
...
function ProductosSection() {
  ...
  const load = useCallback(async () => {
    ...
    setProducts((data ?? []).map((p: Product & { organizations: { name: string } | null }) => ({ ...p, org_name: p.organizations?.name ?? '—' })))
    setLoading(false)
  }, [search])
